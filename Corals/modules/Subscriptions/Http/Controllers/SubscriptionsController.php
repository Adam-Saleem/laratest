...
class SubscriptionsController extends BaseController
{
    ...
    public function __construct(SubscriptionService $subscriptionService)
    {
        ...
        $this->resource_model = new SubscriptionModel();
     ...
     * @param Plan $plan
     * @param User|null $user
     * @return \Illuminate\Foundation\Application|JsonResponse|RedirectResponse|Redirector|mixed
     * @throws Exception
     */
    public function doCheckout(SubscriptionCheckoutRequest $request, Plan $plan, User $user = null)
    {
        $gateway = $request->gateway;
        $subscription_data = $request->only('billing_address', 'shipping_address', 'notes');
        ...
        try {
            if ($plan->exists) {
                ...
                if ($redirectPaymentLink = data_get($user_subscription, 'properties.redirect_payment_url')) {
                    return redirectTo($redirectPaymentLink);
                }
    ...
    /**
     * @param Request $request
     * @param string $subscriptionIdentifierCode
     * @return Application|Factory|View
     */
    public function successPayment(Request $request, string $subscriptionIdentifierCode)
    {
        $subscription = SubscriptionModel::query()->whereRaw(
            "json_unquote(json_extract(properties,'$.subscription_identifier_code')) = ?", [
            $subscriptionIdentifierCode
        ])->first();

        return view('Subscriptions::subscription.success_payment');
    }
     ...
     * @throws Exception
     */
    public function failedPayment(Request $request, SubscriptionService $subscriptionService, string $subscriptionIdentifierCode)
    {
        $subscription = SubscriptionModel::query()->whereRaw(
            "json_unquote(json_extract(properties,'$.subscription_identifier_code')) = ?", [
            $subscriptionIdentifierCode
        ...
        if ($subscription->valid()) {
            $subscriptionService->cancelPlanSubscription(request(), $subscription->plan);
        }


        return view('Subscriptions::subscription.failed_payment')->with(compact('subscription'));
    }
}
