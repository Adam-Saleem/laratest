...
class SubscriptionCycleTransformer extends BaseTransformer
{
    ...
    public function transform(SubscriptionCycle $subscriptionCycle)
    {
        ...
        if (
            ($subscriptionCycle->starts_at && Carbon::make($subscriptionCycle->starts_at)->lte(now()))
            &&
            ($subscriptionCycle->ends_at && Carbon::make($subscriptionCycle->ends_at)->gte(now()))
        ) {
        ...
        $transformedArray = [
            ...
            'subscription' => $subscriptionCycle->subscription->getInvoiceReference("pdf"),
