        ...
        <div class="row">
            <div class="col-md-12">
                @if(user()->canUpdatePaymentDetails())
                    {!! CoralsForm::link(url('subscriptions/payment-configuration'), 'Subscriptions::labels.subscription.payment_configuration', ['class'=>'btn btn-primary']) !!}
                @endif
                {!! CoralsForm::link(url('subscriptions/select'), 'Subscriptions::labels.subscription.add_new_subscription', ['class'=>'btn btn-primary']) !!}
                    ...
                    @forelse(user()->activeSubscriptions() as $subscription)
                        <div class="row m-t-10 m-b-10">
                            <div class="col-md-12">
                                ...
                                @php
                                    $subscriptionDetails = \Filters::do_filter('subscription_details', [
                                      'subscription_reference' => [
                                        'label' =>  trans("Subscriptions::attributes.subscription.sub_reference") ,
                                         'value' => $subscription->subscription_reference
                                      ] ,
                                      'subscription_details' => [
                                        'label' =>  trans("Subscriptions::attributes.subscription.description"),
                        ...
                        @foreach(user()->pendingSubscriptions() as $subscription)
                            <div class="row m-t-10 m-b-10">
                                <div class="col-md-4">
                                    ...
                                    @if($redirectPaymentURL = data_get( $subscription->properties,'redirect_payment_url'))
                                        <p class="alert alert-warning">
                                            @lang('Subscriptions::labels.subscription.complete_your_payment',['url'=>$redirectPaymentURL])
                                        </p>
                                    @endif
                                    @include('Subscriptions::subscription.partials.render_subscription_actions',[
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="table-responsive m-t-10"
                    ...
                    {!! $invoicesDataTable->table(['class' => 'color-table bg-primary table table-hover table-striped table-condensed','style'=>'width:100%;']) !!}
                </div>
            </div>
        </div>
    @endcomponent
    ...
    @component('components.box')
        <div class="row">
            <div class="col-md-12">
                <div class="table-responsive m-t-10"
                     style="min-height: 235px;padding-bottom: 20px;">
                    {!! $subscriptionCyclesDataTable->table(['class' => 'color-table bg-primary table table-hover table-striped table-condensed','style'=>'width:100%;']) !!}
                </div>
            </div>
        </div>
    @endcomponent
